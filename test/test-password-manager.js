...
describe("Password manager", async function () {
	...
	describe("functionality", async function () {
		...
		it("can dump and restore the database", async function () {
			...
			// console.log("new", newKeychain.data.kvs, "old", keychain.data.kvs);
			// console.log("new", newKeychain.data.ivs, "old", keychain.data.ivs);
	...
	describe("security", async function () {
		...
		// swap attack; dump, switch the passwords for two domains, load should throw an error
		// it("fails to load if the data has been tampered with", async function () {
		// 	let keychain = await Keychain.init(password);
		// 	for (let k in kvs) {
		// 		await keychain.set(k, kvs[k]);
		// 	}
		...
		// 	let checksum = data[1];
		// 	let contentsObj = JSON.parse(contents);
		// 	// swap; TODO, this currently doesn't actually swap the values
		// 	let temp = contentsObj.kvs[0];
		// 	contentsObj.kvs.service1 = contentsObj.kvs.service2;
		// 	contentsObj.kvs.service2 = temp;
